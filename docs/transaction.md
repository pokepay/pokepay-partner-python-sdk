...
response = client.send(pp.GetCpmToken(
                          "vn9tkKJsfEeEirDJBvMOLU"                              # cpm_token: CPMトークン
))
```
...
response = client.send(pp.ListTransactions(
                          start="2020-08-18T19:53:03.000000Z",                  # 開始日時
                          to="2021-06-02T09:57:04.000000Z",                     # 終了日時
                          page=1,                                               # ページ番号
...
response = client.send(pp.CreateTransaction(
                          ...
                          money_amount=3272,
                          point_amount=6917,
                          point_expires_at="2022-12-31T07:37:03.000000Z",       # ポイント有効期限
                          description="bJ1KgmPImdwaTBcNwqaqeRCH16a6zzUqrHdosHdbmLywqukvEUDGTtuu5mLHhGQ9yekqoyNLKN2h7BNq3rRMob2yqEgXsKX0DNjA5Llo"
))
```
...
response = client.send(pp.ListTransactionsV2(
                          ...
                          customer_name="太郎",                                   # エンドユーザー名
                          description="店頭QRコードによる支払い",                          # 取引説明文
                          transaction_id="LW2Z",                                # 取引ID
                          is_modified=True,                                     # キャンセルフラグ
                          types=["topup", "payment"],                           # 取引種別 (複数指定可)、チャージ=topup、支払い=payment
                          start="2020-09-03T09:26:31.000000Z",                  # 開始日時
                          to="2020-09-06T02:48:23.000000Z",                     # 終了日時
...
response = client.send(pp.ListBillTransactions(
                          ...
                          is_modified=True,                                     # キャンセルフラグ
                          start="2020-07-05T08:24:28.000000Z",                  # 開始日時
                          to="2021-05-15T18:41:24.000000Z",                     # 終了日時
                          next_page_cursor_id="xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx", # 次ページへ遷移する際に起点となるtransactionのID
                          prev_page_cursor_id="xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx", # 前ページへ遷移する際に起点となるtransactionのID
                          per_page=50                                           # 1ページ分の取引数
))
...
response = client.send(pp.CreateTopupTransaction(
                          ...
                          money_amount=5797,                                    # マネー額
                          point_amount=5521,                                    # ポイント額
                          point_expires_at="2024-01-10T12:48:58.000000Z",       # ポイント有効期限
...
response = client.send(pp.CreateCpmTransaction(
                          "g0EGo2tY0BvAArU4c3Hcr3",                             # cpm_token: CPMトークン
                          "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx",               # shop_id: 店舗ID
                          9863.0,                                               # amount: 取引金額
...
response = client.send(pp.RefundTransaction(
                          ...
                          returning_point_expires_at="2024-12-30T05:44:57.000000Z" # 返却ポイントの有効期限
))
```

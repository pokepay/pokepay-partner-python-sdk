...
response = client.send(pp.ListAccountBalances(
                          "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx",               # account_id: ウォレットID
                          page=9940,                                            # ページ番号
                          per_page=6222,                                        # 1ページ分の取引数
                          expires_at_from="2020-06-08T03:05:33.000000Z",        # 有効期限の期間によるフィルター(開始時点)
                          expires_at_to="2023-06-25T00:03:03.000000Z",          # 有効期限の期間によるフィルター(終了時点)
...
response = client.send(pp.ListAccountExpiredBalances(
                          "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx",               # account_id: ウォレットID
                          page=38,                                              # ページ番号
                          per_page=2785,                                        # 1ページ分の取引数
                          expires_at_from="2020-05-28T14:57:04.000000Z",        # 有効期限の期間によるフィルター(開始時点)
                          expires_at_to="2020-01-19T00:16:50.000000Z",          # 有効期限の期間によるフィルター(終了時点)
                          direction="desc"                                      # 有効期限によるソート順序
))
```
...
response = client.send(pp.UpdateCustomerAccount(
                          ...
                          account_name="kLLtmKfguDK2IgQjODYIDOJbPEulQIvNSkQALktsxpQNr6y6a28m0nRuldHpSuEUpdPie9qQ2GFfC0at9jn8DwInc5YWbNc2E2NkkIcBn5byBGxSlhAbqrppUqGdxMolEMce2oIWkzh6xh3kO5wXHuEli1NcEVyTrbdyJqmh3WRfGT9d54NzUibZax1gbEqwtEhHNUjZJEl7H6aHeFVmJSAKrLNuNDUQhJfNq76RxAuxSV", # アカウント名
                          external_id="rnur4Ju4ayidm5BuCe0yTSEIanUY",           # 外部ID
...
response = client.send(pp.GetCustomerAccounts(
                          ...
                          external_id="YLa0Qhqw2R1myjYzFL4j0H",                 # 外部ID
                          tel="0590-464",                                       # エンドユーザーの電話番号
                          email="vMf7GbuKVO@o81o.com"                           # エンドユーザーのメールアドレス
))
```
...
response = client.send(pp.GetShopAccounts(
                          "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx",               # private_money_id: マネーID
                          page=190,                                             # ページ番号
                          per_page=5600,                                        # 1ページ分のウォレット数
                          created_at_from="2023-12-30T08:30:07.000000Z",        # ウォレット作成日によるフィルター(開始時点)
                          created_at_to="2022-05-27T01:21:55.000000Z",          # ウォレット作成日によるフィルター(終了時点)
...
response = client.send(pp.ListCustomerTransactions(
                          ...
                          start="2022-02-20T12:26:25.000000Z",                  # 開始日時
                          to="2023-09-24T10:58:44.000000Z",                     # 終了日時
                          page=1,                                               # ページ番号

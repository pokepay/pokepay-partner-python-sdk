...
response = client.send(pp.ListCampaigns(
                          ...
                          available_from="2020-01-26T20:15:36.000000Z",         # 指定された日時以降に適用可能期間が含まれているか
                          available_to="2024-09-28T06:01:14.000000Z",           # 指定された日時以前に適用可能期間が含まれているか
                          page=1,                                               # ページ番号
...
response = client.send(pp.CreateCampaign(
                          "dJgT5oXIAxp1c5Q2vG7By91KC2xkwbMvROWfUAhh6XnZz0yJYgRGAM6oTzljbZYS9b6qmrSFaDiVxdn1z0TuA7dLQ8GnuuGnm3um0ZKYlqHYAPfacx4ba4pxXiFCicQd3QQrdtpp5IlW8KnTaroT8w3801Z", # name: キャンペーン名
                          "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx",               # private_money_id: マネーID
                          "2020-01-03T15:21:09.000000Z",                        # starts_at: キャンペーン開始日時
                          "2023-06-28T02:17:06.000000Z",                        # ends_at: キャンペーン終了日時
                          4238,                                                 # priority: キャンペーンの適用優先度
                          "topup",                                              # event: イベント種別
                          bear_point_shop_id="xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx", # ポイント負担先店舗ID
                          description="0FFkkUFLVCDKp9TvCsVFg3Dy6t9FVfvRBKOl2QQeBI5NM6J7EhkzGk22yYle2ZOPXJOiEYcNwwBKhoxCdqw8SDS6L7O6ohLm8HBuYz7E9ZuYBAHz0vH45u4SHdXpfYeqMtcfd8wxcygIW1kAzyAHjkW0eFslSf8NaBTyV6GBT8tDHI0zWcr0sMpkdiHOOwl5x", # キャンペーンの説明文
                          status="disabled",                                    # キャンペーン作成時の状態
                          point_expires_at="2023-02-20T20:48:36.000000Z",       # ポイント有効期限(絶対日時指定)
                          point_expires_in_days=4798,                           # ポイント有効期限(相対日数指定)
  ...
  "to": "23:59"
                          ...
                          minimum_number_of_products=3943,                      # キャンペーンを適用する1会計内の商品個数の下限
                          minimum_number_of_amount=7335,                        # キャンペーンを適用する1会計内の商品総額の下限
                          minimum_number_for_combination_purchase=4016,         # 複数種類の商品を同時購入するときの商品種別数の下限
                          exist_in_each_product_groups=True,                    # 複数の商品グループにつき1種類以上の商品購入によって発火するキャンペーンの指定フラグ
                          max_point_amount=3497,                                # キャンペーンによって付与されるポイントの上限
                          max_total_point_amount=3200,                          # キャンペーンによって付与されるの1人当たりの累計ポイントの上限
  ...
  "value": "bronze"
                          ...
                          budget_caps_amount=216711518                          # キャンペーン予算上限
))
```
...
response = client.send(pp.UpdateCampaign(
                          "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx",               # campaign_id: キャンペーンID
                          name="uFUQK5yc0JqyEbk4xV1ElwOVpwOgCs3R",              # キャンペーン名
                          starts_at="2024-05-01T05:20:02.000000Z",              # キャンペーン開始日時
                          ends_at="2022-09-09T12:58:48.000000Z",                # キャンペーン終了日時
                          priority=3116,                                        # キャンペーンの適用優先度
                          event="topup",                                        # イベント種別
                          description="EJ",                                     # キャンペーンの説明文
                          status="enabled",                                     # キャンペーン作成時の状態
                          point_expires_at="2021-09-03T14:21:32.000000Z",       # ポイント有効期限(絶対日時指定)
                          point_expires_in_days=6407,                           # ポイント有効期限(相対日数指定)
  ...
  "to": "23:59"
                          ...
                          applicable_shop_ids=["xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx"], # キャンペーン適用対象となる店舗IDのリスト
                          blacklisted_shop_ids=["xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx", "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx", "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx"], # キャンペーン適用対象外となる店舗IDのリスト(ブラックリスト方式)
                          minimum_number_of_products=7893,                      # キャンペーンを適用する1会計内の商品個数の下限
                          minimum_number_of_amount=6901,                        # キャンペーンを適用する1会計内の商品総額の下限
                          minimum_number_for_combination_purchase=4989,         # 複数種類の商品を同時購入するときの商品種別数の下限
                          exist_in_each_product_groups=True,                    # 複数の商品グループにつき1種類以上の商品購入によって発火するキャンペーンの指定フラグ
                          max_point_amount=8614,                                # キャンペーンによって付与されるポイントの上限
                          max_total_point_amount=4445,                          # キャンペーンによって付与されるの1人当たりの累計ポイントの上限
                          applicable_account_metadata={
  "key": "sex",
  "value": "male"
},                                                                              # ウォレットに紐付くメタデータが特定の値を持つときにのみ発火するキャンペーンを登録します。
  ...
  "value": "bronze"
                          ...
                          budget_caps_amount=1727543161                         # キャンペーン予算上限
))
```
